use api::types::{ApiTransactionResponse, InitializeOrApply, Transfer, Withdraw, WrapTokens};
...
pub async fn withdraw(
    ...
    for (idx, mut tx) in txs.into_iter().enumerate() {
        loop {
            ...
            if idx == 0 {
                tx.sign(&vec![&key, &equality_proof_keypair, &range_proof_keypair], blockhash);
            } else {
