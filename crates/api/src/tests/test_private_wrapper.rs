...
async fn test_wrap() {
    ...
    let wrapped_mint = test_client
        .create_confidential_wrapped_mint(&key, &unwrapped_mint)
        .await;

    // create unwrapped token account
    test_client
        .create_token_account(&key, &unwrapped_mint)
        .await;
    ...
    test_client.test_apply(&key, wrapped_mint).await;
